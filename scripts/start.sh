...
usage() {
  ...
  echo "        label:          The value to set in the metadata of the container that uniquely identifies it. Example \"label=my.unique.container\" "
...
TS=$(date '+%Y%m%d-%H%M%S')
volume=${volume:-cms_${INTERFACE}_volume-$TS}
docker create \
...
if [ "${label:-X}" != 'X' ]; then
  DOCKER_RUN_CMD="${DOCKER_RUN_CMD} --label friendly_name=${label}"
fi
if [ "${port:-X}" = 'X' ]; then
